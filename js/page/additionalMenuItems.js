...
function addThemeMain(theme) {
	if (theme.hasOwnProperty('version') && theme['version'] == 2) {
		...
		if (theme['name'].endsWith('.mp4')) {
			themeFrame = `<iframe id="roproThemeFrame" src="https://api.ropro.io/themeFrameVideo.php?theme=${theme['name']}${theme.hasOwnProperty('h') ? `&h=${parseInt(theme['h'])}&s=${parseFloat(theme['s'])}&l=${parseFloat(theme['l'])}` : ''}" frameborder="0" style="top:0;left:0;position:absolute;border:0;width:100%;height:100%;z-index:-1;"></iframe>`
		} else {
...
async function insertMenuItems() {
	...
	newButtonHTML = '<a class="themes-icon dynamic-overflow-container text-nav" href="/themes" id="nav-inventory"><div><span style="transform:scale(1.2);margin-left:1px;" class="new-menu-icon icon-nav-charactercustomizer"></span></div><span class="font-header-2 dynamic-ellipsis-item">Themes</span></a>'
