...
async function fetchServerInfo(placeID, servers) {
	return new Promise(resolve => {
		chrome.runtime.sendMessage({greeting: "PostURL", url:"https://ropro-proxy.deno.dev/getServerInfo.php///api", jsonData: {'placeID':placeID, 'servers': servers}}, 
			function(data) {
				resolve(data)
			}
		)
...
function createUpgradeModalServerFilters() {
    ...
    <a href="https://ropro.io#plus" target="_blank"><button type="button" style="font-family:HCo Gotham SSm;position:absolute;left:25px;top:440px;width:450px;" class="btn-growth-sm PurchaseButton">Subscribe - $3.99/month</button></a>
    </div></div>`
    modalDiv.innerHTML += modalHTML
    body = document.getElementsByTagName('body')[0]
    body.insertBefore(modalDiv, body.childNodes[0])
