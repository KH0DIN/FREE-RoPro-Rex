...
async function fetchServerInfo(placeID, servers) {
	return new Promise(resolve => {
		$.post({url:"https://ropro-proxy.deno.dev/getServerInfo.php///api", data: {'placeID':placeID, 'servers': servers}}, 
			function(data) {
				resolve(data)
			}
		)
...
async function fetchServerConnectionScore(placeID, servers) {
	return new Promise(resolve => {
		$.post({url:"https://ropro-proxy.deno.dev/getServerConnectionScore.php///api", data: {'placeID':placeID, 'servers': servers}}, 
			function(data) {
				resolve(data)
			}
		)
...
async function fetchServerAge(placeID, servers) {
	return new Promise(resolve => {
		$.post({url:"https://ropro-proxy.deno.dev/getServerAge.php///api", data: {'placeID':placeID, 'servers': servers}}, 
			function(data) {
				resolve(data)
			}
		)
...
async function launchCloudPlayTab(placeID, serverID = null, accessCode = null) {
	...
	$.get('https://api.ropro.io/analytics.php?event=cloud_play&qualifier=' + (placeID + ',' + await getStorage("rpUserID")))
}
	...
	function fetchSubscription() {
		return new Promise(resolve => {
			async function doGet(resolve) {
				...
				$.post({url:"https://ropro-proxy.deno.dev/getSubscription.php///api?key=" + await getStorage("subscriptionKey") + "&userid=" + userID, headers: {'ropro-verification': roproVerificationToken, 'ropro-id': userID}}, function(data){
					subscription = data
	...
	const validateLicense = () => {
			$.get('https://users.roblox.com/v1/users/authenticated', function(d1, e1, r1) {
					...
					async function doValidate() {
						...
						$.ajax({
							url:'https://ropro-proxy.deno.dev/validateUser.php///api' + freeTrial,
							type:'POST',
							headers: {'ropro-verification': roproVerificationToken, 'ropro-id': userID},
